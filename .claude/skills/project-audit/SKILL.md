...
For this project specifically: coordinates are WGS84 lat/lon (EPSG:4326)
straight from Socrata, and deck.gl's `HexagonLayer` takes them as such — there
is no reprojection step, so the hazard is not a missing `.to_crs()` but a
**hex radius in metres binning degree-spaced points**, and any future area or
distance figure computed in degrees². Also in scope: `dow` derived from a
naive `date_created` with no stated timezone.
...
### Module independence
Each `pipeline/` module runnable standalone: own/argued paths, no top-level state
...
A paged or capped API truncates silently. Check that the downloader verifies
its count against the live server count and fails hard — not that it merely
stopped when a page came back short. See `docs/DATA.md`.

### DATA.md currency
Column names, row counts, quirks, exclusions — all matching reality. A stale
`docs/DATA.md` means knowledge is leaking between sessions; flag it.
