...
1. **Never `Read` raw data files.** A single GeoJSON can be millions of tokens —
   ...
   a few rows) — never the raw bytes. In this repo that means, specifically:
   **`site/data/complaints.geojson`** (~14.6k features, ~2 MB — one feature per
   complaint) and **`data/raw_complaints.json`** (the uncommitted fetch cache).
   `site/data/meta.json` is small and safe to read.
...
4. **Keep modules small and single-purpose.** One processing step per file in
   `pipeline/`, ≤~250 lines. If a module crosses ~400 lines, that's the signal
   to split.
...
- `site/data/complaints.geojson` — the big one. Never read raw; summarize.
- `data/raw_complaints.json` — uncommitted fetch cache, same hazard.
